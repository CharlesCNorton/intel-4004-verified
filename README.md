...
A Hoare logic framework provides pre and postcondition specifications: per-instruction rules for the register, accumulator, branch, call/return, and RAM-character operations, and one value-level rule whose postcondition is the data-sheet reference function, covering the fifteen-instruction ALU class at once. The remaining I/O and port instructions are specified by execute-level theorems in `behavior.v` rather than Hoare rules. Frame rules establish non-interference properties for register file modifications, RAM updates, and accumulator changes. Consequence rules enable pre and postcondition strengthening and weakening. Compositional reasoning principles support sequential program verification. The framework includes verified example programs demonstrating register value copying, accumulator computation pipelines, subroutine invocation with the ring call/return discipline, and RAM read-modify-write sequences.
