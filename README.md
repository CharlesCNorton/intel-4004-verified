...
No MCS-4 document defines a return past the ring's depth; the model gives it the silicon meaning, a stale row resuming as the PC (`bbl_underflow_from_reset`, `ring_underflow_agrees`), rather than leaving it unspecified. The stale row holds the address one past the return that vacated it, because the row is incremented during the return's own fetch and abandoned without a write-back; this detail was recovered by differential testing against the transistor netlist (`netlistval/`), which caught the model's original convention (the unincremented address) disagreeing with the masks at exactly the fourth-deep return.
...
**netlistval/**: the transistor-netlist validation harness: a switch-level simulator over the mask-derived netlist, the model-driven vector generator, and the bus-level comparator (`netlistval/README.md`).
...
make netlistval # differential test against the mask-derived transistor netlist
```
...
- Formal gate-level equivalence against the 4004 netlist. The differential half is done: `netlistval/` drives a switch-level simulation of the mask-derived netlist through the bus protocol and matches the model on ~86,000 observed fields; a machine-checked equivalence proof between the netlist and the ISA model would be a standalone project on top of it.
- Validation against physical 4004 silicon. The emulator cross-validation covers the ALU exhaustively plus the control-flow, stack, and RAM suites (`crossval/`), the netlist validation covers the CPU at its pins, and the vector dumps double as bench vector sets; the remaining step needs a 4004 on the bench (the one question that requires real hardware beyond the CPU is the multi-line DCL read, which is analog contention between 4002s).
